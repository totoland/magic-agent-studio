 ...
 *   { kind:'error', text }
 */
export function runAgent({ task, body, model, tools, cwd, sharedContext, resumeSessionId }, onEvent) {
  ...
  if (resumeSessionId) args.push("--resume", resumeSessionId); // continue an existing chat session
  if (model && model !== "inherit") args.push("--model", model);
  // On resume the session already carries its system prompt — don't re-append it
  // (would duplicate). On a fresh run/turn, prepend shared team context (stable,
  // cache-eligible) then the agent's persona.
  if (!resumeSessionId) {
    const appended = sharedContext ? `${sharedContext}\n\n---\n\n${body || ""}` : (body || "");
  ...
  let sessionId = null; // captured from the stream so chat turns can --resume it
  let finished = false;
  let stderr = "";
  ...
  function handleLine(line) {
    ...
    if (evt.session_id && !sessionId) { sessionId = evt.session_id; onEvent({ kind: "session", id: sessionId }); }
