 ...
 *   { kind:'error', text }
 */
export function runAgent({ task, body, model, tools, cwd }, onEvent) {
  ...
  child.on("close", (code) => {
    ...
    if (code !== 0) {
      ...
      onEvent({ kind: "done", status: "completed", tokens });
    }
  });
  ...
  return {
    stop() {
      ...
      finished = true;
      child.kill("SIGTERM");
      onEvent({ kind: "done", status: "stopped", tokens });
    },
  };
}
