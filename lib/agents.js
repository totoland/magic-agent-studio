...
function metaFor(id, meta, index) {
  ...
  return {
    ...
    spriteHeight: m.spriteHeight || null, // front-row height in vh for the Office (null = default)
  };
}
...
async function saveMetaFields(id, { color, glyph, avatar, spriteHeight }) {
  const meta = await readMeta();
  const prev = meta[id] || {};
  const next = { ...prev };
  ...
  if (spriteHeight !== undefined) { if (spriteHeight) next.spriteHeight = spriteHeight; else delete next.spriteHeight; }
  meta[id] = next;
  await writeMeta(meta);
}

export async function setSpriteHeight(id, height) {
  await saveMetaFields(id, { spriteHeight: height ? Number(height) : null });
  return getAgent(id);
}
